...
with sync_playwright() as p:
    ...
    # 找下载链接 (pdf / word)
    dls = page.evaluate('''() => Array.from(document.querySelectorAll('a, button')).map(a => [(a.innerText || '').trim(), a.href || '']).filter(x => x[0].includes('下载') || x[0].includes('Word') || x[0].includes('PDF') || x[1].endsWith('.pdf') || x[1].endsWith('.docx') || x[1].endsWith('.doc'))''')
    print("Download links:", dls)
    b.close()
