...
with sync_playwright() as p:
    ...
    # 监听 response
    requests_log = []
    page.on("response", lambda res: requests_log.append((res.url, res.status)))
    
    page.goto(url, timeout=30000)
    page.wait_for_timeout(3000)
    
    print("Logged API requests:")
    for u, st in requests_log:
        if 'api' in u or 'detail' in u or 'download' in u or 'flk' in u:
            print("  ", st, u)
